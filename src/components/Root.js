...
class Root extends Component {
 ...
 // console.log("in load canvas",item.expInput);
 ...
 case 2:
 // console.log("storycard before ",this.state.storyCardObj);
 var SliderStatus = true;
 ...
 deleteInput(index,e,id) {
    ...
    () => {
    const tempdel = (this.state.storyCardObj.filter(item => item.inputListId === index));
    tempdel.map((item) => {
      if(item.expVariable !== null) {
        this.deleteInputFromStorycard(item.inputListId);
